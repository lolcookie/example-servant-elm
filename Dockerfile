# Basic nix install based on alpine
FROM nixos/nix

RUN nix-channel --add https://nixos.org/channels/nixpkgs-unstable nixpkgs
RUN nix-channel --update

# RUN nix-build '<nixpkgs>' -A elmPackages.elm
...
#   haskellPackages.reserve               \
#   haskellPackages.sensei                \
#   pkgconfig             \
#   stack                 \
#   elmPackages.elm      \
...
# # RUN apk update \
# #   && apk add nodejs \
# #   && npm install -g elm-test --unsafe-perm=true --allow-root \
# #   && apk add make \
